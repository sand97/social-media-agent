...
async function main() {
    ...
    console.log(`✅ WhatsApp Agent created: ${whatsappAgent.ipAddress}:${whatsappAgent.port}`);
    console.log('🏷️  Creating system tags...');
    const systemTags = [
        { name: 'IA désactivée', color: '#EF4444', isSystem: true, userId: null },
        { name: 'Réponse manuelle requise', color: '#F59E0B', isSystem: true, userId: null },
        { name: 'Promesse de commande', color: '#10B981', isSystem: true, userId: null },
    ...
    const systemGroups = [
        { name: "Groupe d'amélioration", type: 'IMPROVEMENT', isSystem: true, userId: null },
        { name: 'Groupe de commandes', type: 'ORDERS', isSystem: true, userId: null },
        { name: 'Groupe de support', type: 'SUPPORT', isSystem: true, userId: null },
    ];
    ...
    await prisma.productMetadata.createMany({
        data: [
            { productId: product1.id, key: 'taille', value: 'M', isVisible: true, suggestedByAI: false },
            { productId: product1.id, key: 'matière', value: '100% coton', isVisible: true, suggestedByAI: false },
        ],
    });
    ...
    await prisma.deliveryLocation.createMany({
        data: [
            { userId: testUser.id, country: 'France', city: 'Paris', name: 'Centre-ville', price: 5.0, isActive: true },
            { userId: testUser.id, country: 'France', city: 'Paris', name: 'Banlieue', price: 8.0, isActive: true },
        ],
    });
