...
import { Alert, Button, Empty, Progress, Skeleton, message, Typography } from 'antd'
import {
  lazy,
  Suspense,
  useEffect,
  ...
  return (
    <>
      <DashboardHeader
        ...
        right={<Skeleton.Button active size='default' style={{ width: 136 }} />}
      />
...
export default function CatalogPage() {
  ...
  const [isIndexingAlertDismissed, setIsIndexingAlertDismissed] =
    useState(false)
  const catalogQuery = useQuery<CatalogData>({
    queryKey: ['catalog'],
    queryFn: () => catalogApi.getCatalog(),
  })
  const catalogData = catalogQuery.data ?? null
  const isInitialLoading = catalogQuery.isPending && !catalogData
  const isBackgroundLoading = catalogQuery.isFetching && !!catalogData
  ...
  return (
    <>
      ...
      />
      <div className='flex flex-col gap-6 w-full py-6 px-6'>
        {catalogQuery.isError && !catalogData ? (
          <Alert
            type='error'
            showIcon
