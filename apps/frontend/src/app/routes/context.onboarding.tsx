...
const ONBOARDING_THREAD_QUERY_KEY = ['onboarding-thread'] as const
...
async function getOnboardingThread(): Promise<OnboardingThread | null> {
  try {
    const response = await apiClient.get<OnboardingThread>('/onboarding/threads')
...
function removeLastUserMessage(messages: ThreadMessage[]) {
  ...
  const lastUserMessage = messages[lastUserIndex] ?? null
...
export default function ContextOnboardingPage() {
  ...
  const cachedThreadData =
    queryClient.getQueryData<OnboardingThread | null>(
      ONBOARDING_THREAD_QUERY_KEY
    ) ?? undefined
  const cachedThread = cachedThreadData ?? null
  const hasCachedThread = cachedThreadData !== undefined && cachedThread !== null
  const [inputValue, setInputValue] = useState('')
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [thread, setThread] = useState<OnboardingThread | null>(cachedThread)
  const [messages, setMessages] = useState<ThreadMessage[]>(
    cachedThread?.messages ?? []
  )
  const [score, setScore] = useState(cachedThread?.score ?? user?.contextScore ?? 0)
  ...
  const [viewMode, setViewMode] = useState<'chat' | 'summary'>(() =>
    cachedThread?.score && cachedThread.score >= 80 && cachedThread.context?.trim()
      ? 'summary'
      : 'chat'
  )
  const [expandedSections, setExpandedSections] = useState<string[]>([])
  const socketRef = useRef<Socket | null>(null)
  const threadRef = useRef<OnboardingThread | null>(cachedThread)
  ...
  const initialViewModeSetRef = useRef(hasCachedThread)
  ...
  const threadQuery = useQuery({
    queryKey: ONBOARDING_THREAD_QUERY_KEY,
    queryFn: getOnboardingThread,
  })
  const isInitialLoading =
    threadQuery.isPending && threadQuery.data === undefined
  const isBackgroundLoading =
    threadQuery.isFetching &&
    !isInitialLoading &&
    (thread !== null ||
      messages.length > 0 ||
      Boolean(contextMarkdown.trim()))
  ...
  }, [score, hasShownModal, isInitialLoading, user?.agentConfig])
  ...
  useEffect(() => {
    ...
    if (threadQuery.data) {
      ...
      setContextMarkdown(threadQuery.data.context || '')
      handleScoreUpdate(threadQuery.data.score || 0)
      setInitialViewMode(threadQuery.data.score || 0, threadQuery.data.context)
      return
    }
  ...
  useEffect(() => {
    ...
    console.error('Failed to fetch thread:', threadQuery.error)
    setInitialViewMode(initialUserScoreRef.current, '')
  }, [threadQuery.error, threadQuery.isError])

  useEffect(() => {
    const updateCachedThread = (
      updater: (current: OnboardingThread) => OnboardingThread
    ) => {
      setThread(current => {
        if (!current) {
          return current
      ...
      queryClient.setQueryData<OnboardingThread | null>(
        ONBOARDING_THREAD_QUERY_KEY,
        current => {
          if (!current) {
            return current
    ...
    socket.on(
      ...
      (data: { success: boolean; restoredContent: string | null }) => {
        ...
        setMessages(prev => removeLastUserMessage(prev).nextMessages)
        updateCachedThread(current => ({
          ...current,
          messages: removeLastUserMessage(current.messages).nextMessages,
        }))
      }
    )
    ...
    socket.on(
      ...
      (data: { message: string; retryable: boolean; type: string }) => {
        ...
        if (data.retryable) {
          setMessages(prev => {
            const { lastUserMessage, nextMessages } = removeLastUserMessage(prev)
